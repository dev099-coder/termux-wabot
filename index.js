...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Em processo aguarde by dev Java ⌛',
				success: '✔️ ativado ✔️',
				error: {
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado pelo ademir do grupo! ❌',
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Esta é a lista de números bloqueados porque fez merda. :\n'
					for (deixe o bloqueio do bloqueado) {
						teks += `~> @${block.split('@')[0]}\n`
					}
				...
				case 'meme':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'aqui esta teu meme irmão'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/${memein.hash}.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'aqui esta teu meme indo irmão'})
					break
				case 'setprefix':
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A marca-alvo que você deseja chutar da nave!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Toma otario quem mandou ser desobediente com o ademir remente que foi removido:\n'
						...
						mentions(`Toma otario quem mandou ser desobediente com o ademir remente que foi removido: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					...
					no = 0
					for (deixe o administrador do grupo Admins) {
						no += 1
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responda com uma sticker! ❌')
					reply(mess.wait)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativando o modo simi com sucesso neste grupo ✔️')
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('O recurso de boas-vindas foi desativado com sucesso neste grupo ✔️')
					} else {
